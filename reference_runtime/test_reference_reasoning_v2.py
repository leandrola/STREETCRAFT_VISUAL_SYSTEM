...
class ReferenceReasoningTests(unittest.TestCase):
    ...
    def run_engine(self, needs, items=None, **kw):
        ...
        args.update(kw);return resolve_reference_v2(**args)
    ...
    def test_cgc_copy_and_block(self):
        c={'preserve':['geometry']};r=self.run_engine([self.need()]);o=enrich_cgc_v2(c,r);o['preserve'].clear();self.assertEqual(c['preserve'],['geometry'])
        with self.assertRaises(ValueError):enrich_cgc_v2(c,self.run_engine([self.need()],[]))
