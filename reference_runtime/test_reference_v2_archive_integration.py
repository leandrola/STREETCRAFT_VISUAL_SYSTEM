...
from reference_reasoning_v2 import resolve_reference_v2,enrich_cgc_v2
...
def run(ns,items,**kw):
    return resolve_reference_v2(needs=ns,profile='VP03',mode='T06',camera='CG-FC',archive_retriever=make_archive_v1_retriever(archive,items),fear_city_confirmed=True,**kw)
...
c={'preserve':['authored geometry'],'forbid':['regional import']};out=enrich_cgc_v2(c,r);check('cgc_locks_preserved',out['preserve']==c['preserve'] and out['forbid']==c['forbid'],r)
...
out=Path(__file__).resolve().parents[1]/'validation/REFERENCE_V2_ARCHIVE_INTEGRATION.json';out.write_text(json.dumps(report,indent=2)+'\n')
