...
def resolve_entity_action(entity:dict, relationships:list[dict], mode:str)->dict:
    ...
    if pl=="P0":
        ...
        action="PRESERVE_CHARACTER"
    elif pl=="P2":
        action="PRESERVE_CONTEXT"
    elif pl=="P3":
        action="TRANSFORM_SCOPED"
    elif pl=="P4":
        action="REMOVE_AUTHORIZED" if removal_authorized else "PRESERVE_CONTEXT"
    elif pl in {"P5-A","P5-B"}:
        action="INFER_MINIMAL" if multiview or entity.get("continuity_support",False) else "UNKNOWN_LOCKED"
    elif pl=="P5-C":
        action="INFER_MINIMAL" if multiview else "UNKNOWN_LOCKED"
    elif pl=="P5-D":
        action="UNKNOWN_LOCKED"
    else:
        raise ValueError(f"unknown preservation level {pl}")

    # T01 observed transient content remains evidence.
    if mode=="T01" and observed and ("TRANSIENT_OBJECT" in roles or "OCCLUDER" in roles):
        if pl not in {"P0","P1"}:
            action="PRESERVE_CONTEXT"

    # Removal needs explicit authority even in T03.
    if mode=="T03" and ("REMOVAL_CANDIDATE" in roles or "TRANSIENT_OBJECT" in roles):
        if removal_authorized and pl=="P4":
            action="REMOVE_AUTHORIZED"

    # Fear City authored geometry stays locked.
    if mode=="T06" and authored and ("STRUCTURAL_CORE" in roles or pl=="P0"):
        action="PRESERVE_EXACT"

    # Protected relationship outranks removal or broad transform.
    if relation_locked and action in {"REMOVE_AUTHORIZED","TRANSFORM_SCOPED"}:
        action="PRESERVE_RELATIONSHIP"

    # Unknown region stays unknown.
    ...
    return {
      ...
      "action":action,
      "relationship_lock":relation_locked,
      "protected_relationships":locks["relationships"],
      "salience":salience_score(entity)
    }
...
def validate_relationships(entities:list[dict], relationships:list[dict])->list[str]:
    ...
    for r in relationships:
        if r["subject"] not in ids or r["object"] not in ids:
            warnings.append(f"DANGLING_RELATIONSHIP:{r['relationship_id']}")
        if r.get("protection")=="PR0" and r.get("epistemic_class")=="UNKNOWN":
            warnings.append(f"PR0_UNKNOWN_EVIDENCE:{r['relationship_id']}")
    return warnings
    ...
    for a in actions:
        ...
        if action=="UNKNOWN_LOCKED":
            hints.append({
              ...
              "state":"RN_BLOCKED",
              "reason":"Scene evidence is locked unknown; generic references cannot create exact content."
            })
        elif action=="TRANSFORM_SCOPED" and e.get("reference_domain"):
            hints.append({
              "entity_id":e["entity_id"],
              "state":"RN_REQUIRED" if e.get("reference_required",False) else "RN_SUPPORT",
              "target_domains":[e["reference_domain"]],
              "reason":"Scoped transformation may require documentary implementation evidence."
            })
        elif action=="INFER_MINIMAL" and not e.get("multi_view_support",False):
            hints.append({
              "entity_id":e["entity_id"],
              "state":"RN_SUPPORT",
              "reason":"Minimum reconstruction may benefit from scoped evidence, but source continuity remains primary."
            })
    return hints

def build_scene_analysis_record(*, scene_id:str, source_identity:str, mode:str, profile:str,
                                entities:list[dict], relationships:list[dict])->dict:
    ...
    return {
      ...
      "reference_need_hints":hints,
      "warnings":warnings
    }

def project_scene_to_cgc(cgc:dict, sar2:dict)->dict:
    out=dict(cgc)
    entities={e["entity_id"]:e for e in sar2["entities"]}
    preserve=[]; remove=[]; infer=[]; unknown=[]; forbid=[]
    for a in sar2["action_plan"]:
        eid=a["entity_id"]; label=entities[eid].get("label",eid)
        act=a["action"]
        if act in {"PRESERVE_EXACT","PRESERVE_CHARACTER","PRESERVE_CONTEXT","PRESERVE_RELATIONSHIP"}:
            preserve.append(f"{eid}:{label}:{act}")
        elif act=="REMOVE_AUTHORIZED":
            remove.append(f"{eid}:{label}")
        elif act=="INFER_MINIMAL":
            infer.append(f"{eid}:{label}:MINIMAL")
        elif act=="UNKNOWN_LOCKED":
            unknown.append(f"{eid}:{label}")
            forbid.append(f"resolve_exact_unknown:{eid}")
        elif act=="FORBID_CHANGE":
            forbid.append(f"change:{eid}")
        if a.get("relationship_lock"):
            forbid.append(f"break_protected_relationships:{eid}")
    out["preserve"]=list(dict.fromkeys(out.get("preserve",[])+preserve))
    out["remove"]=list(dict.fromkeys(out.get("remove",[])+remove))
    out["infer"]=list(dict.fromkeys(out.get("infer",[])+infer))
    out["unknown"]=list(dict.fromkeys(out.get("unknown",[])+unknown))
    out["forbid"]=list(dict.fromkeys(out.get("forbid",[])+forbid))
    out["scene_intelligence"]={
      "scene_id":sar2["scene_id"],
      "identity_anchors":sar2["identity_anchors"],
      "protected_relationships":[r for r in sar2["relationships"] if r.get("protection") in {"PR0","PR1"}],
      "entity_actions":sar2["action_plan"],
      "unknown_locks":sar2["unknown_locks"],
      "reference_need_hints":sar2["reference_need_hints"]
    }
    return out
